...
class Member extends UtilFile {
	...
	function findPassword($reqData) {
        ...
        $arrMember = $this->objDBH->getRow("select id,email from ".$this->table." where id='".$reqData['id']."' and name='".$reqData['name']."' and status='y'");
		if ($arrMember['id']) {
            // 임시 비번 생성
	...
	function updateAdmin($reqData) {
        ...
        // 비번 변경시
        if (!empty($reqData['current_password'])) {
            $arrMember = $this->objDBH->getRow("select password from ".$this->table." where code='".$reqData['member_code']."'");
            if ($arrMember['password'] != getHash($reqData['current_password'])) {
                returnData(_API_FAIL, "기존 비밀번호가 일치하지 않습니다.");
            }
            // 비밀번호 자리수 체크
