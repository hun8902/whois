...
function getLoginCode() {
	return $_SESSION['login_code'];
}

function getNonLoginCode() {	// 비회원 로그인시 non_member_code (code값)
	return $_SESSION['non_login_code'];
}
...
function getLoginLevel() {
	if ($_SESSION['login_level']) return $_SESSION['login_level'];
	else {
        $arrMyConfig = getCFG("MyConfig");
        return $arrMyConfig['LevelType']['non_member'];
    }
}
...
function getPageInfo() {
	...
	$template_path = _TEMPLATE_ROOT."/".implode("/",$arrDir) ."/".$file."."._APPL_TEMPLATE_EXTENSION;
...
function returnData($code=_API_SUCCESS, $data="") {
    if (isset($_POST['method']) or checkAccessMobile()) {    // Mobile 접근
